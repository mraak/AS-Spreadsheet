	...
	public class Calc extends EventDispatcher
	{
		...
		public function solveExpression(exp:String, forget:Boolean = true):String
		{
			...
			if (isValid == "ok")
			{
				...
				if(!forget) repairedExpression = Utils.repairExpression(exp);
			...
			if(Utils.isString(exp)) exp = Utils.stripStringQuotes(exp);
		...
		private function solveParen():String
		{
			...
			else if (a.length == 0)
			{
				...
				if(!Utils.isString(exp)) exp = getValueFromOperand(exp).toString();
			}
			else
			{
		...
		public function getCtrlById(ctrlId:String):ControlObject
		{
			...
			if (ignoreCase) ctrlId = ctrlId.toLowerCase();
			...
			if(currentTarget) if(currentTarget.grid) if(currentTarget.grid.ctrlObjects[ctrlId.toLowerCase()]) ro = currentTarget.grid.ctrlObjects[ctrlId.toLowerCase()];
		...
		public function assignControlExpression(ctrl:*, expression:String, update:Boolean = false):void
		{
			...
			else if(exp == "")
			{
				...
				if(!update)
				{
					...
					if(co.grid) co.grid.expressionTree.splice(co.grid.expressionTree.indexOf(co),1);
				} 
		...
		* */
		...
		{			
			...
			if(!copy) assignControlExpression(fromField, "");
		...
		private function onCtrlFocus(evt:Event):void
		{
			...
			if (co.ctrl is TextInput)
			{
				if(co.exp != null && co.exp != "") co.ctrl[co.valueProp] = co.exp;
			} 
		}
		...
		public function addSpreadsheet(sheet:ISpreadsheet):void
		{
			...
			if(sheet.calc != this) sheet.calc = this;
