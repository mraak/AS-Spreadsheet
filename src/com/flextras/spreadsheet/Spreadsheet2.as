		...
		*/
			...
			if(expressionsChanged)
			{
				...
				if(_expressions is ArrayCollection)
				{
					...
					for each(var o:Object in _expressions)
					{
						...
						if(usedCells.indexOf(cell) != -1) throw(new Error("Cell ID already used in this collection: " + cell + ". Use setItem() or itemUpdated() if you want to change existing cell's expression.")); 
		...
		protected function createRowsAndColumns():void
		{
			...
			for (j = 0; j < c; j++)
			{
				...
				if(this.columnWidth) dc.width = this.columnWidth;
				cols.push(dc);
			}
